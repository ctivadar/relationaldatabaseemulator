...
public:
    ...
    using Only_allow_for_table_types_t =
...
private:
    ...
    */
    template <typename T>
    using is_fast_pass = std::integral_constant<bool, std::is_scalar<typename std::decay<T>::type>::value>;
    template <typename T>
    using fast_pass = typename std::conditional<
      is_fast_pass<typename std::decay<T>::type>::value, typename std::decay<T>::type,
      typename std::add_lvalue_reference<typename std::add_const<typename std::decay<T>::type>::type>::type>::type;
...
bool TableEntry::compare(const TableEntry& other) const noexcept {
    assert(tag == other.tag && "tried to compare TableEntry containing different types");
    switch (tag) {
...
// Helper to dedupe constructor code
template <typename TT>
void TableEntry::construct_from(TT&& other) {
    switch (tag) {
    case EntryType::String:
